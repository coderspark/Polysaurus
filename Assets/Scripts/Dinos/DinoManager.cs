    ...
    public void SpawnDinos()
    {
        foreach(Dino dino in Dinos){
            for (int i = 1; i <= dino.spawn_rate; i++){
                ...
                dinoscript.Set(dino.DinoSpeed, dino.DinoAgroDistance, dino.DinoIdleDistance, dino.DinoName, dino.DinoAgroType, dino.Health, dino.DinoAttackDistance, dino.DinoType, dino.AttackDamage);
            }
        }
    }
}
